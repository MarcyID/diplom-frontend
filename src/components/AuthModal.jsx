...
function AuthModal({ setIsLoggedIn, setUser }) {
    ...
    const handleSubmit = (e) => {
        ...
        // 1. Запоминаем, что пользователь вошел
        setIsLoggedIn(true)

        // 2. Если это регистрация, сохраняем имя. Если вход — оставляем как есть (или обновляем)
        if (!isLogin && name) {
            setUser(prev => ({ ...prev, name: name }))
        } else if (isLogin && name) {
            // Опционально: обновить имя и при входе, если поле заполнено
            setUser(prev => ({ ...prev, name: name }))
        }
