...
function DirectorSearchModal({ isOpen, onClose, onMovieClick, onDirectorClick }) {
    ...
    const handleOpenCard = () => {
        ...
        if (onDirectorClick && personId) {
            onDirectorClick(personId)
        }
        // Не закрываем окно поиска - карточка откроется поверх
    }

    // Фильтруем фильмы где персона была режиссёром и сортируем (сначала general=true)
    const directorFilms = (filmography || [])
        .filter(f => f.professionKey === 'DIRECTOR')
                            ...
                            {!selectedPerson ? (
                                ...
                                <div className="director-search-view">
                                    ...
                                    <div className="directors-list">
                                        ...
                                        {!loading && persons.length > 0 && persons.map(person => (
                                            <button key={person.personId || person.id} className="director-card" onClick={() => handlePersonSelect(person)}>
                                                ...
                                                <div className="director-info">
                                                    <span className="director-name">{person.nameRu || person.nameEn}</span>
                                                </div>
                                        ...
                                        {!loading && !searchQuery.trim() && (
                                            <div className="empty-state">
                                                ...
                                                <p>Введите имя режиссёра для поиска</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="director-filmography">
                                    <div className="director-profile">
                                        ...
                                        <div>
                                            <h3 className="director-profile-name">{selectedPerson.nameRu || selectedPerson.nameEn}</h3>
                                            {personDetails?.profession && (
                                                <span className="director-profile-profession">{personDetails.profession}</span>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="filmography-section">
                                        ...
                                        <div className="filmography-list">
                                            {!personDetails ? (
                                                ...
                                                <p className="empty-films">Нет фильмов этого режиссёра</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
